''' Derived from libs/main_WILD_Inference_AudioMotion_PerParticipant.py'''
import time
import sys
...
def eval_one_epoch(model, loader, criterion, epoch, logger, args):
    ...
    # # y_pred[y_pred== 5] = 3
    # # y_pred[y_pred == 12] = 13
    # _,inx = torch.topk(torch.from_numpy(y_prob), 3, 1)
    # hits = (y_true == inx.numpy()[:, :3].T).any(axis=0)
    # inx = inx.numpy()
    # y_pred = [y_true[i] if hits[i] else inx[i,0] for i in range(len(hits))]
    # y_pred = np.concatenate(y_pred, 0)
...
if __name__ == '__main__':
    ...
    # YVETTE - this will be our individual data...
    WILD_participants = []

    if os.path.exists('./Data/collected_data_rawAudioSegmentedData_window_' + str(win_size) + '_hop_' + str(hop) + '_2Sessions.pkl'):   #(old data with only 1 session, os.path.exists('../Data/WILD_JW_HH_KH_LV_SK_rawAudioSegmentedData_window_' + str(win_size) + '_hop_' + str(hop) + 'NEW.pkl'):  
        with open('./Data/collected_data_rawAudioSegmentedData_window_' + str(win_size) + '_hop_' + str(hop) + '_2Sessions.pkl', 'rb') as f:
            WILD_participants = pickle.load(f)
    else:
        start = time.time()
        files = ['YE']#['JW','HH','KH','LV','SK']
        for pname in files:
            print('participant ', pname, ' data read...')
            p = parti(pname, './Data/' + pname, win_size, hop)
            p.readRawAudioMotionData()
            WILD_participants.append(p)
        end = time.time()
        print("time for feature extraction: " + str(end - start))

        with open('./Data/collected_data_rawAudioSegmentedData_window_' + str(win_size) + '_hop_' + str(hop) + '_2Sessions.pkl', 'wb') as f:
            pickle.dump(WILD_participants, f)


    model_name = 'AttendDiscriminate_MotionAudio_CNN14_Concatenate'
    experiment = f'WILD_Inference_{model_name}_PerParticipant'
    ...
    config_model = {
        ...
        "sa_div": 1,
        "num_class": 23,
        "train_mode": False, # YVETTE - using pretrained model so set this to False (originally set to True)
    ...
    # results.writerow(["Participant", "Precision", "Recall", "F-Score", "Accuracy", "F_weighted"])
    ...
    for u in participants[-1:]:
        ...
        for x in WILD_participants:
            X_testM = np.vstack((x.rawMdataX_s1, x.rawMdataX_s2))
            X_testA = np.vstack((x.rawAdataX_s1, x.rawAdataX_s2))
            y_test = np.vstack((x.rawdataY_s1, x.rawdataY_s2))
            X_testM = X_testM[(y_test != 23)[:,0]]
            X_testA = X_testA[(y_test != 23)[:,0]]

            y_test = y_test[y_test != 23]
            print("participant : " + x.name)
            args['participant'] = x.name
    ...
    plot_confusion(Y_True, Y_Pred, None, 0, normalize=False, cmap=plt.cm.Blues, class_map=[chr(a+97).upper() for a in clss])
    plt.show()
    # file.close()
