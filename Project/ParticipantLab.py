...
class ParticipantLab:
    ...
    def __init__(self, name, folder, win_size, overlap, normalized=False):
        ...
        for i in range(1,3):
        	self.a_folder.append(folder + '/P' + name + '/' + name + str(i) + '/SegAudio/')
        	self.m_folder.append(motion_folder + '/P' + name + '/' + name + str(i) + '/SegMotion/')
        self.a_folder_s1 = folder + '/P' + name + '/' + name + '1/SegAudio/'
        self.m_folder_s1 = motion_folder + '/P' + name + '/' + name + '1/SegMotion/'
        self.a_folder_s2 = folder + '/P' + name + '/' + name + '2/SegAudio/'
        self.m_folder_s2 = motion_folder + '/P' + name + '/' + name + '2/SegMotion/'
        self.data_s1 = []
        self.data_s2 = []
        self.rawMdataX_s1 = []
        self.rawAdataX_s1 = []
	...
	# sample rate
        ...
        # session 1
	# for test, a_folder_s1= "../Data/P01/011/SegAudio"
        if os.path.exists(self.a_folder_s1):
            ...
            for index in range(0,len(a_files)):
                ...
                a_matrix = librosa.util.frame(x=audio, frame_length=win_audio, hop_length=hop_length_audio)
        ...
        #### THIS V V V IS DIFFERENT FROM SESSION2 BELOW (splits 2nd dimension at specific point)
        ...
        if os.path.exists(self.a_folder_s2): 
            ...
            for index in range(0,len(a_files)):
                ...
                # If length of audio matrix is longer than motion matrix, cut down audio matrix or v.v.
                if(a>m):
                    row = m
                    a_matrix = a_matrix[:,:row]
                else:
                ...
                # Grab label from filename (letter designates activity)
                # Translate ascii to unicode. unicode(97)=a (eg.x=23)
                if self.name == 'Test':
                    label = ord(a_files[index][4])-97
                else:
                    label = ord(a_files[index][3])-97
